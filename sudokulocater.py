...
class locater():
    def main(self):
        ...
        if self.sudoku is not None:
            ...
            self.img = cv2.imread('sudokubrd_cap.png')
            self.grayimg = self.canny()
            self.cpImg = self.canny.copy()
            self.colorConvImg = cv2.cvtColor(self.img, cv2.COLOR_RGB2BGR)
            cv2.imwrite('sudokubrd_canny.png', self.grayimg)
            slice('sudokubrd_canny.png', 9*9,)
    ...
    def boardFinder(self):
        time.sleep(0.5)
        self.sudoku = pg.locateOnScreen('boardtest.PNG', confidence=0.6)
        print(self.sudoku)
        self.sudoku_b = pg.screenshot(region=self.sudoku)
        self.sudoku_b.save('sudokubrd_cap.png')
        return self.sudoku
    ...
    def numDetector(self):
        ...
        for col in range(9):
            ...
            for row in range(9):
                print('row'+str(x))
                self.compimg = cv2.imread('sudokubrd_canny_0' + str(y) + '_0' + str(x) + '.png')
